...
/**
 * 借阅业务 — 对接 day3 LibraryService 借还逻辑
 */
@Service
public class LoanService {
    ...
    public Loan borrowBook(LoanRequest request) {
        ...
        if (!memberRepository.existsById(request.memberId())) {
            throw new BusinessException("MEMBER_NOT_FOUND", "会员不存在: " + request.memberId());
        }
    ...
    public ReturnResponse returnBook(LoanRequest request) {
        Loan loan = loanRepository
                .findByBookIdAndMemberIdAndReturnDateIsNull(request.bookId(), request.memberId())
